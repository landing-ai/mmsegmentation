...
class VanillaAttentionWithSRRatio(BaseModule):
    def __init__(
        ...
        attn_drop=0.,
        proj_drop=0.,
        dropout_layer=None,  # Ignoring this! The user has no reason to change the dropout layer
        init_cfg=None,
        batch_first=True,  # This should always be true for this drop-in
        qkv_bias=False,
        ...
        if not batch_first:
            raise NotImplementedError("For this drop-in we only support batch_first format. Sorry.")

        self.embed_dims = embed_dims
        self.num_heads = num_heads
        head_dim = embed_dims // num_heads
        self.scale = head_dim**-0.5  # Use a hard-coded scale to match the API from mmseg - no reason to change this
    ...
    def forward(self, x, hw_shape, identity=None):
        ...
        if identity is not None:
            x = x + identity  # Identity is really the skip connection
...
class TransformerEncoderLayer(BaseModule):
    ...
    def __init__(self,
        ...
        self.attn = VanillaAttentionWithSRRatio(
