...
class Attention(nn.Module):
    def __init__(
        ...
        if lora_dim:
            self.q = lora.Linear(dim, dim, r=lora_dim)
            self.kv = lora.MergedLinear(
                dim,
                dim * 2,
                r=lora_dim,
...
class MixVisionTransformerV2(nn.Module):
    def __init__(
        ...
        self.fuse_head = SegFormerFuse(
            in_channels=self.embed_dims, embedding_dim=self.fuse_dim
        )

        self.summarizer_c3 = Summarizer(self.embed_dims[-2])
        self.summarizer_c4 = Summarizer(self.embed_dims[-1])
        self.summarizer_f = Summarizer(self.fuse_dim)
...
class Summarizer(nn.Module):
    def __init__(self, in_dim, depth=1, num_heads=8):
        super().__init__()
        self.cls_token = nn.Parameter(torch.zeros(1, 1, in_dim))
        trunc_normal_(self.cls_token, std=0.02)
        self.blks = nn.ModuleList(
            [
                ClassAttention(in_dim, num_heads=num_heads, qkv_bias=True)
...
class mit_b2(MixVisionTransformer):
    def __init__(self, lora_dim=0, **kwargs):
        super(mit_b2, self).__init__(
            embed_dims=[64, 128, 384, 384],
            fuse_dim=384,
            num_heads=[1, 2, 6, 6],
            mlp_ratios=[4, 4, 4, 4],
            qkv_bias=True,
